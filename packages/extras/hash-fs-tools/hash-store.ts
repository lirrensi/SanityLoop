...
import { hashStorePath, hashStoreDir, legacyHashStorePath } from "./paths.ts";
import { errCode, isRec, splitLines } from "./utils.ts";
import { initHasher, contentChecksum } from "./hashline/hasher.ts";
import { HASH_RE } from "./hashline/alphabet.ts";
import { HASH_STORE_VERSION, HASH_STORE_BUSY_TIMEOUT } from "./constants.ts";
...
interface Prepared {
  ...
  undoUpsert(path: string, content: string, bom: string, ending: string, hashes: string, resultContent: string, updatedAt: number): void;
  undoGet(path: string): Record<string, unknown> | undefined;
  undoDelete(path: string): void;
  servedGet(path: string): Record<string, unknown> | undefined;
  servedUpsert(path: string, hashes: string, updatedAt: number): void;
  servedDelete(path: string): void;
}
...
/**
 * A fully in-memory HashStore — no files, no home folder. The default for the
 * factory: read/replace/undo work completely in-session (anchors, undo, served
 * tracking) with zero disk writes. Pass `storeDir` or your own `store` to
 * persist.
 */
export function createMemoryHashStore(): HashStore {
  const snapshots = new Map<string, { checksum: string; lineCount: number; hashes: string }>();
  const undo = new Map<string, { content: string; bom: string; ending: string; hashes: string; resultContent: string }>();
  const served = new Map<string, string>();

  const stmts: Prepared = {
    get: (path: string, checksum: string, lineCount: number) => {
      const s = snapshots.get(path);
      if (!s || s.checksum !== checksum || s.lineCount !== lineCount) return undefined;
      return { hashes: s.hashes };
    },
    allPaths: () => [...new Set([...snapshots.keys(), ...undo.keys(), ...served.keys()])].map((path) => ({ path })),
    allHashes: () => [...snapshots.entries()].map(([path, s]) => ({ path, hashes: s.hashes })),
    deleteOne: (path: string) => void snapshots.delete(path),
    upsert: (path: string, checksum: string, lineCount: number, hashes: string) => void snapshots.set(path, { checksum, lineCount, hashes }),
    undoUpsert: (path: string, content: string, bom: string, ending: string, hashes: string, resultContent: string) =>
      void undo.set(path, { content, bom, ending, hashes, resultContent }),
    undoGet: (path: string) => {
      const u = undo.get(path);
      return u ? { content: u.content, bom: u.bom, ending: u.ending, hashes: u.hashes, result_content: u.resultContent } : undefined;
    },
    undoDelete: (path: string) => void undo.delete(path),
    servedGet: (path: string) => {
      const h = served.get(path);
      return h !== undefined ? { hashes: h } : undefined;
    },
    servedUpsert: (path: string, hashes: string) => void served.set(path, hashes),
    servedDelete: (path: string) => void served.delete(path),
  };
  return { stmts, engine: "node:sqlite" };
}

/** Factory knob: force loadHashStore() to return a specific store. */
let overrideStore: HashStore | undefined;
export function setHashStore(store: HashStore | undefined): void {
  overrideStore = store;
}

export function loadHashStore(): Promise<HashStore> {
  if (overrideStore) return Promise.resolve(overrideStore);
