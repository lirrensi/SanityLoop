import { abortIf, rejectUnknownFields, firstNonEmptyIndex, lastNonEmptyIndex, clipLine } from "../utils.ts";
import { HASH_CLASS, HASH_SEP, HL_BARE_PREFIX_RE, HL_PREFIX_PLUS_RE, HL_PREFIX_MINUS_RE, canon } from "./hash.ts";
import { parseHashRef, parseText, type Anchor } from "./parse.ts";
import { NEW_CONTENT_NOT_STRING_MSG, MAX_RANGE_STALE_LINES } from "../constants.ts";
...
export function valEdit(
	...
	if (!startResolved || !endResolved) {
		if (!startResolved && endResolved) {
			const startMismatch = [...mismatches].reverse().find((m) => m.ref === edit.hash_bounds[0]);
			if (startMismatch && startMismatch.kind === "not_found") startMismatch.context = endResolved;
		} else if (startResolved && !endResolved) {
			const endMismatch = [...mismatches].reverse().find((m) => m.ref === edit.hash_bounds[1]);
